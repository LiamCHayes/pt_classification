...
setwd("/Users/ethanstraub/Downloads/pt_classification-master")
```
...
## Method 1: Machine Learning Methods with Regression Parameters

There are 45 time series for every exercise that a subject does. A machine learning method needs some kind of information from each of these series that it can use to differentiate between the exercises. 

From looking at the graphs of the series, some general trends can be seen: the acceleration and gyroscope sensors that are supposed to be moving while the exercise is being performed have high variance clear signals, while the acceleration and gyroscope sensors that are not intended to move while the exercise is being performed have low variance and unclear signals. For the magnemometer measurements, the signals are clear with high variance when the muscle that the sensor is on is contracting while the signals are unclear with low variance when the muscle that the sensor is on is not supposed to be contracting. 

A measurement that captures how clear and high variance a signal is the amplitude of a sin regression. A sin regresison will fit with a higher amplitude when the signals are clearer and variance is higher. 

Here are graphs of clear and unclear signals. You can see that the fitted sin regression has a larger amplitude in the series with clear signals compared to the series with poor signals that are mostly just noise.

There are some issues from some series that could cause this strategy to fail. Here is the graph of a series that would cause problems:

```{r fig.cap="Original Series"}
knitr::include_graphics("plots/non_stationary.png")
```

If a sin regression was fit to this series, it would roughly follow the mean of the series. The amplitude from this sin regresison would not reflect the signals from each repetition of the exercise. To solve this, I fit a spline to each series and used the residuals to obtain a detrended version of each series that a sin regression could fit to follow the repetitions well. I set the penalty term ($\lambda$) to be 0.05 so that the splines don't underfit or overfit to the data.

Here is a graph of the same series with the a spline fit to it:

```{r fig.cap="Original Series with Spline"}
knitr::include_graphics("plots/original_series.png")
```

Here is a graph of the original series compared to the detrended series. You can see how the constant mean of the detrended series makes it a better option to fit a sin regression to.

```{r fig.cap="Detrended Series"}
knitr::include_graphics("plots/detrended_series.png")
```

This detrending works great until the series is too short because the splines do not fit through the mean of the series. Therefore, I fit the sin regression to the original series rather than the detrended series when we are using increments of less than 15 seconds. Here's an example of this. You can see that a sin regresison would fit better to the original series compared to the detrended series.

```{r fig.cap="Detrending a Short Series"}
knitr::include_graphics("plots/short_series.png")
```

The next graphic is a glimpse of a feature matrix made from the amplitudes of the sin regressions that were fitted to the detrended versions of each series. The full matrix is 40 rows by 45 columns. Each row represents a subject performing an exercise (5 subjects x 8 exercises = 40 rows). Each column represents a measurement from a sensor (5 sensors x 9 measurements per sensor = 45 measurements).

```{r fig.cap="Feature Matrix"}
knitr::include_graphics("plots/table.png")
```

We used this matrix as the feature matrix and the corresponding exercise numbers as the labels in a few classification methods: k-nearest-neighbors, support vector machines, and random forests. To find the best parameters for these methods, I used 5 fold cross validation where each fold is all of the data from 1 subject. This way, a subject is not evaluated on a classifier that was trained using other exercises from that subject. The results of these evaluations are posted further in the paper.
...
Using k-nearest neighbors, the best parameters are: weights = uniform (each nearest neighbor contributes the same weight in the classification decision), k = 1 (1 neighbor), and p = 1 (p = 1 means the distances are manhattan distances). The estimated accuracy of this classifier is 0.675.

Using support vector machines, the best parameters are: C = 100 (regularization parameter), and kernel = linear. The estimated accuracy of this classifier is also 0.675.

Using random forests, the best parameters are: n_estimators = 50 (50 decision trees in the forest). The estimated accuracy of this classifier is 0.8.

We also tried using the variances of the series instead of the amplitudes of fitted sin regressions in the feature matrix with a random forest model. The estimated accuracy of this classifier is also 0.775.

The feature matrices so far used the 5 sensor numbers (1 through 5) to differentiate between the sensors. But if we use sensor locations instead of sensor numbers, there are 8 sensor locations (5 for lower body exercises and 5 for upper body exercises. But 2 of the sensors are in the same location for both upper and lower body exercises). The next random forest classifier gives the classifier information on sensor locations by filling in zeroes when there is no sensor at a given location during the exercise. This classifier has an estimated accuracy of 0.875. 0.875 is the best accuracy that we were able to get using machine learning methods. Here is a confusion matrix from this classifier. 

```{r fig.cap="Confusion Matrix"}
knitr::include_graphics("plots/conf_mat.png")
```

All of the classifiers so far had had a training accuracy of 100% while the test accuracies lagged far behind. This is not surprising given the fact that the feature matrices have more columns than rows. We tried using principal component analysis in an attempt to stop overfitting to the training set, but accuracy did not improve. 

```{r fig.cap="Principal Component Analysis"}
knitr::include_graphics("PCA.png")
```

We also tried using smaller time intervals or less sensor measurements in the feature matrices. But this did not improve accuracy either. Note that in the next table, the classifiers that used smaller time intervals used intervals where the patients were doing high quality reps.

```{r fig.cap="Other Methods"}
knitr::include_graphics("other.png")
```

With so few subjects, a more deterministic method like Dynamic Time Warping can outperform random forests.
