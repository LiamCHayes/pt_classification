...
From looking at the graphs of the series, some general trends can be seen: the acceleration and gyroscope sensors that are supposed to be moving while the exercise is being performed have clear, high variance signals. The acceleration and gyroscope sensors that are not intended to move while the exercise is being performed have unclear, low variance signals. For the magnemometer measurements, the signals have high variance when the muscle that the sensor is on is contracting while the signals have low variance when the muscle that the sensor is on is not supposed to be contracting. 

A measurement that captures how clear and high variance a signal is the amplitude of a fitted sin regression. Here are graphs of clear and unclear signals with a sin regression fitted to it. You can see that the sin regression has a larger amplitude in the series with clear signals compared to the series with poor signals that are mostly just noise.

```{r fig.cap="Fitting Sin Regression"}
knitr::include_graphics("plots/sin_reg.png")
```
...
If a sin regression was fit to this series, it would roughly follow the mean of the series. The amplitude from this sin regression would not reflect the signals from each repetition of the exercise. To solve this,  a spline was fitted to each series and used the residuals to obtain a detrended version of each series that a sin regression could fit to follow the repetitions well. The penalty term for these splines ($\lambda$) was set at 0.05 so that the splines don't underfit or overfit to the data.
...
This detrending works great until the series is too short because the splines do not fit through the mean of the series. Therefore, we usethe original series rather than the detrended series when we are using increments of less than 15 seconds. Here's an example of this. You can see that a sin regresison would fit better to the original series compared to the detrended series.

```{r fig.cap="Detrending a Short Series"}
knitr::include_graphics("plots/short_series.png")
```

The next graphic is a glimpse of a feature matrix made from the amplitudes of the sin regressions that were fitted to the detrended versions of each series. The full matrix is 40 rows by 45 columns. Each row represents a subject performing an exercise (5 subjects x 8 exercises = 40). Each column represents a measurement from a sensor (5 sensors x 9 measurements per sensor = 45).

```{r fig.cap="Feature Matrix"}
knitr::include_graphics("plots/table.png")
```

We used this matrix as the feature matrix and the corresponding exercise numbers as the labels in a few classification methods: k-nearest-neighbors, support vector machines, and random forests. The results of these methods are posted further in the paper.
...
To find the best parameters for these methods, we used 5 fold cross validation where each fold is all of the data from 1 subject. This way, a subject is not evaluated on a classifier that was trained using other exercises from that subject. 
